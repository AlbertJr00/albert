...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t©©© Saran Ektensi : [green]Cookiedoufb[white] ©©©'))
...
def crack_file():
	...
	if len(vin)==0:
		...
		try:geh = lol[geeh]
			...
			print(f'{k}>> Enter Yang Bener Kontol {x}')
			time.sleep(3)
			back()
...
#			id2.insert(xx,bacot)
		...
		print('>> Enter Yang Bener Kontooll ')
		exit()
	print('>> 1. P ')
	print('>> 2. Mbasic ')
	#print('>> 3. Touch  )
...
def passwrd():
	...
	print('')
	print(f'>> Hasil {h}OK{x} Saving : {h}OK/%s {x}'%(okc))
	print(f'>> Hasil {k}CP{x} Saving : {m}CP/%s {x}'%(cpc))
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r🔥 {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{M}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
